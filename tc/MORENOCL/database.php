<?php
	...
	class database
	{
		...
		//---------------------------------------
		private $db_name_qas = 'postgres';
		private $db_name_prd = 'ohwnggei_postgres';
		//---------------------------------------
		private $db_user_qas = 'root';
		private $db_user_prd = 'ohwnggei_postgres';
		//---------------------------------------
		private $db_pass_qas = '';
		private $db_pass_prd = 'cW3dOcZYN%BEW=RGDV5%^z*qV';
		//---------------------------------------
		...
		//---------------------------------------------------------PREDETERMINADO
			public function get_datos($pid,$type,$db='con'){
				...
				switch ($type) {
					case 'tc':
						$sql = "SELECT * FROM tc WHERE id_tc=".$pid." LIMIT 1 ;";
					break;
					case 'tc_first':
						$sql = "SELECT * FROM tc ORDER BY id_tc ASC LIMIT 1 ;";
					break;
					case 'tc_last':
						$sql = "SELECT * FROM tc ORDER BY id_tc DESC LIMIT 1 ;";
					break;
					case 'tc_day':
						$sql = "SELECT * FROM tc WHERE fecha LIKE '".$pid."' LIMIT 1 ;";
					break;
					default:
						$sql = null;
					break;
